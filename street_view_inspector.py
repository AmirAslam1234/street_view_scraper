...
class streetViewSpider(scrapy.Spider):
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(streetViewSpider, cls).from_crawler(crawler, *args, **kwargs)
        crawler.signals.connect(spider.spider_closed, signal=signals.spider_closed)
        return spider

    def start_requests(self):
        db_query = pd.read_sql("SELECT * FROM query WHERE isDone = 0",self.db)
    ...
    def parse_page(self,response):
        ...
        for i in new_data[0][1]:
            if len(i) <=11:
                ...
                updated_url = main_url[0]+ "/@"+ ",".join(latitude_longitude) + ",17z/data" + main_url[-1]
                ...
                yield scrapy.Request(url=linkn,callback=self.parse_info,meta={"query_id":query_id},dont_filter=True)
